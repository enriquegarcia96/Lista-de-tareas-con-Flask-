...
from todo import db
from todo.auth import login_required #protege todos los endpoints 
...
def index():
    ...
    todos = c.fetchall()
    return render_template('todo/index.html', todos=todos)#le paso las tareas de la consulta a esta plantilla
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'select t.id, t.description, t.completed, t.created_by, u.username '
...
@login_required
def update(id):
    todo = get_todo(id) #-> le paso a una funcion el id del todo
